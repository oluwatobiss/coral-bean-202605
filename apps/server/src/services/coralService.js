...
export async function getRecentEmails() {
    ...
    const cached = coralCache.get(cacheKey);
    if (cached)
        return cached;
    try {
        const rawData = await runCoralCommand("SELECT * FROM gmail.emails ORDER BY timestamp DESC LIMIT 15");
        // Normalize Coral output to match our strict Email interface
        const emails = rawData.map((row) => ({
...
export async function getUpcomingEvents() {
    ...
    try {
        ...
        const rawData = await runCoralCommand("SELECT * FROM calendar.events WHERE start >= CURRENT_TIMESTAMP ORDER BY start ASC LIMIT 15");
...
//# sourceMappingURL=coralService.js.map