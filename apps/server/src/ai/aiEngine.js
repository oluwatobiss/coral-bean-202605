import { extractInsights, buildTimeline, generateMetrics, generateMemory, chatHeuristic } from "./heuristicEngine.js";
// Flag to switch to LLM engine in the future when API keys are available
const USE_LLM = false;
export async function processDashboardData(emails, events) {
...
export async function processChat(query, emails, events) {
    if (USE_LLM) {
        // return await chatLLM(query, events, emails);
    }
    return chatHeuristic(query, events, emails);
}
export function getActiveNotifications(emails, events) {
    const insights = extractInsights(emails, events);
